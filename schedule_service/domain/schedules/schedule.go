...
func (s *Schedule) SetWeekday(weekday time.Weekday) {
	s.weekday = weekday
}

func (s *Schedule) SetLessonNumber(lessonNumber int32) {
	s.lessonNumber = lessonNumber
}

func (s *Schedule) validate() error {
	if s.weekday == time.Saturday || s.weekday == time.Sunday {
		return fmt.Errorf("%w: alesson cannot be scheduled for saturday and sunday", ErrInvalidScheduleData)
	}
